...
stats_folder = os.path.join(outpath, f'Statistics_bundlesizelim_distance_{str(distance)}{space_param}{inclusive_str}{symmetric_str}{folder_ratio_str}{other_spec}')
...
for group_select in group_selects:
    ...
    for target_tuple in target_tuples:
        ...
        size_largest_nc_bundles = [num for num in selected_sizes[non_control] if num > np.max([selected_sizes[non_control][0]/3,20])]

        dist_all = np.zeros((num_bundles, np.size(size_largest_nc_bundles)))
        ...
        for g3 in np.arange(num_bundles):
            for g4 in np.arange(np.size(size_largest_nc_bundles)):
        ...
        for j,group in enumerate(groups):
            group_list[group]=[dist_all_idx[i][j] for i in range(num_bundles)]
        ...
        if calc_distance_csv:
            ...
            for group in groups:
                ...
                for i in np.arange(num_bundles_group):
                    for j in np.arange(num_bundles_group):
                        try:
